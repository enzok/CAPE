...
@route("/tasks/create/quarantine", method="POST")
@route("/v1/tasks/create/quarantine", method="POST")
def tasks_create_quarantine():
    response = {}
    ...
    temp_file_path = store_temp_file(data.file.read(), data.filename)
    qpath = unquarantine(temp_file_path)
    ...
    if not qpath:
        return HTTPError(404, "Not a valid quarantine file.")

    try:
        task_ids = db.demux_sample_and_add_to_db(file_path=qpath, package=package, timeout=timeout, options=options, priority=priority,
                machine=machine, platform=platform, custom=custom, memory=memory, enforce_timeout=enforce_timeout, tags=tags, clock=clock,
                shrike_url=shrike_url, shrike_msg=shrike_msg, shrike_sid=shrike_sid, shrike_refer=shrike_refer)
    except CuckooDemuxError as e:
        return HTTPError(500, e)
