...
urlpatterns = [
    ...
    url(r"^zipdownload/(?P<category>\w+)/(?P<task_id>\d+)/(?P<dlfile>\w+)/$", analysis_views.zipdownload, name='zipdownload'),
    ...
    url(r"^guacamole/(?P<task_id>\d+)/tunnel/(?P<host>\w+)/$", guac_views.tunnel, name='tunnel'),
